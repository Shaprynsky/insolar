...
func (h *MessageHandler) createRedirect(
	ctx context.Context, genericMsg core.Parcel, msg *message.GetObject, definedState *core.RecordID, pulse core.PulseNumber,
) (*reply.GetObjectRedirectReply, error) {
	var (
		toNodes []core.RecordRef
		err     error
	)
	if definedState != nil && pulse-definedState.Pulse() < h.conf.LightChainLimit {
		// Find light executor that saved the state.
		toNodes, err = h.JetCoordinator.QueryRole(ctx, core.RoleLightExecutor, &msg.Head, definedState.Pulse())
		if err != nil {
			return nil, err
		}
	} else {
		// Find heavy that has this object.
		toNodes, err = h.JetCoordinator.QueryRole(ctx, core.RoleLightExecutor, &msg.Head, msg.Head.Record().Pulse())
		if err != nil {
			return nil, err
		}
	}
	...
	redirect := reply.NewGetObjectRedirectReply(&toNodes[0], definedState)
